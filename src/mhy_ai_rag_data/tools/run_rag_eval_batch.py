...
def parse_retriever_output(out: str) -> Dict[str, Any]:
    ...
    for line in out.splitlines():
        ...
        if s.startswith("retrieved="):
            try: res["retrieved"] = int(s.split("=",1)[1].strip())
...
def keyword_score(text: str, keywords: List[str]) -> Dict[str, Any]:
    hit=0; misses=[]
    for kw in keywords:
        if kw and kw in text: hit+=1
        else: misses.append(kw)
...
def main() -> int:
    ...
    report={"generated_at": datetime.now().isoformat(timespec="seconds"),
            ...
            "k": args.k, "pipeline": bool(args.pipeline), "answer": bool(args.answer),
            "results": []}

    for it in items:
        qid=it.get("id"); q=it.get("query"); kws=it.get("expect_keywords",[]) or []
        if not q: continue
