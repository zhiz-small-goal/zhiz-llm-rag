...
- [1) 详细指导](#1-详细指导)
  ...
  - [Step2 — 将进度/WAL 升级为“写入多少、记录就同步在”](#step2-将进度wal-升级为写入多少记录就同步在-con)
  - [Step3 — changed/deleted 的可重入顺序与幂等](#step3-changeddeleted-的可重入顺序与幂等-done)
  - [Step4 — 一致性校验与故障可复盘](#step4-一致性校验与故障可复盘-done)
  - [Step5 — 测试矩阵与回归（含中断注入）](#step5-测试矩阵与回归含中断注入-done)
  - [Step6（可选）— 警告收敛与门禁策略固化](#step6可选-警告收敛与门禁策略固化-done)
- [自检](#自检)
- [失败模式与缓解](#失败模式与缓解)
- [MRE](#mre)
- [替代方案](#替代方案)
