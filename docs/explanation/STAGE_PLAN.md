...
### [009] 2026-01-11 ?? repo health ?????check_repo_health_files?
...
**???**  
- ?? Public Release Preflight ? repo health ??????????  
- ????????/?????????  

**?????**  
- ????? `tools/check_repo_health_files.py` ????? preflight ????
...
**?????**  
- src/mhy_ai_rag_data/tools/check_repo_health_files.py  
  - stdlib-only ?? repo health ??? JSON ??  
  - ?? `result=...` / `required_missing=...` / `placeholders=...` / `report_written=...`
- tools/check_repo_health_files.py  
  - wrapper ??? src ????  
- tools/check_repo_health_files_README.md  
  - ??????????/??????  
...
**?????**  
- [ ] ?? `python tools/check_repo_health_files.py --repo . --mode public-release --out data_processed/build_reports/repo_health_report.json`??? `result=PASS` ?????  
