...
def main() -> int:
    ...
    # 2) extract units if needed
    # Rationale: inventory.csv is the source of truth; if it changes, units MUST be regenerated.
    inv_path = (root / "inventory.csv").resolve()
    force_extract = str(args.force_extract_units).lower() in {"1", "true", "yes", "y", "on"}
